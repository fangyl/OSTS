...
function top2(){
   	form3.action="<%=basePath%>flinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>flinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>flinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>flinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>flinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>flinfo.jsp?page="+page;
      form3.submit();
    ...
    <TD vAlign=top align=left width=760 height=547>
      <TABLE class=border2 height=250 cellSpacing=0 cellPadding=0 width="100%" border=0>
        ...
        <TR>
          ...
          <IMG height=11 src="<%=basePath%>images/up.gif" width=11></SPAN><STRONG> 分类查看 </STRONG></TD></TR>
<%		String flm=Common.toChineseAndTrim(request.getParameter("flm"));
    cb.setEVERYPAGENUM(5);
	int cou = cb.getMessageCount("select count(*) from sp where pp='"+flm+"'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from sp where pp='"+flm+"' order by id desc",9);
...
<TABLE align=center cellSpacing=0 cellPadding=0  border=0>
  <TBODY>
  <TR>
    <TD align=left>  <input type="hidden" name="flm" value="<%= flm%>" /><!--//用于给上面javascript传值-->
