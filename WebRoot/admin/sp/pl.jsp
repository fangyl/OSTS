...
function top2(){
   	form3.action="<%=basePath%>admin/sp/pl.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/sp/pl.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/sp/pl.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/sp/pl.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/sp/pl.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/sp/pl.jsp?page="+page;
      form3.submit();
...
<%
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from pl order by id desc",6);
...
%> 
    <TR class="css_col11">
      <td height="23"><%=i+1 %></td>
      <td><%=cb.getString("select mc from sp where id='"+pagelist2.get(1).toString()+"'") %></td>
      <td><%=pagelist2.get(2).toString() %></td>
      <td><%=pagelist2.get(3).toString() %></td>
      <td><%=pagelist2.get(4).toString() %></td> 
      <td><a href="<%=basePath%>delPl.action?method=delpl&id=<%=pagelist2.get(0).toString()%>" >删除</a></td>
    </tr>
<% }} %> 
  <TR class="css_col11"> 
				<TD colspan=10>
